    ...
    <div class="row no-gutters margin-bottom-20">
        <div class="col-12 bg-white">
            ...
            <form action="{{ route('users.profile')}}" class="padding-30" method="post">
                ...
                <x-input type="text" name="email" placeholder="ایمیل" value="{{ auth()->user()->email }}"
                         class="text-left" required/>
                <x-input type="text" name="mobile" placeholder="موبایل" value="{{ auth()->user()->phone }}"
                         class="text-left"/>
                ...
                @can("teach")
                  <template>
                        ...
                        <x-input type="text" name="card_number" placeholder="شماره کارت بانکی"
                                 value="{{ auth()->user()->card_number }}" class="text-left"/>
                        <x-input type="text" name="shaba" placeholder="شماره شبا بانکی"
                                 value="{{ auth()->user()->shaba }}"
                                 class="text-left"/>
                        <x-input type="text" name="username" placeholder="نام کاربری و آدرس پروفایل"
                                 value="{{ auth()->user()->username }}" class="text-left"/>
