...
@section('content')
    <div class="main-content">
        <form action="{{ route("settlements.store")  }}" method="post" class="padding-30 bg-white font-size-14">
            @csrf
            <x-input name="name"  type="text" placeholder="نام صاحب حساب" class="text" />
            <x-input name="cart"  type="text" placeholder="شماره کارت" class="text" />
            <x-input name="amount" type="text" placeholder="مبلغ به تومان" class="text" value="{{auth()->user()->balance}}" required/>
